...
If needed, then re-run the first chunk of `PondSplit.Rmd` as well.
...
f1a.i <- plot_grid(ye, mo, ncol = 2, align = "hv")
...
N <- ggplot() +
  ...
  scale_fill_viridis_c(option = "plasma", name = "N years\nsampled") +
  xlim(-70.65, -69.95) +
  ylim(41.55, 42.07) +
  theme_classic() +
  theme(legend.title = element_text(family = "mont", color = "#193446"),
        legend.text = element_text(family = "open", color = "#676767"),
...
L <- ggplot() +
  ...
  geom_point(data = years, aes(x = Long, y = Lat, fill = last_year),
             shape = 21, size = 1.25, stroke = 0.25) +
  scale_fill_viridis_c(option = "viridis", name = "Last year\nsampled") +
  xlim(-70.65, -69.95) +
  ylim(41.55, 42.07) +
  theme_classic() +
  theme(legend.title = element_text(family = "mont", color = "#193446"),
...
f1a.iii <- plot_grid(N, L, ncol = 2, align = "hv")

ggsave("figures/f1a.iii.png", f1a.iii, width = 6.5, height = 3, units = "in", dpi = 600)
ggsave("figures/svg/f1a.iii.svg", f1a.iii, width = 6.5, height = 3, units = "in")
```
...
allSurface <- read.csv("data/AllPonds_Surface.csv") %>%
  ...
  mutate(DevelopmentClass = factor(DevelopmentClass,
...
# handle legends until cowplot bug fixed
...
surf_list <- lapply(params, function(param) {
  ggplot() +
    ...
    theme(panel.grid = element_blank(),
          ...
          legend.title = element_text(hjust = 0.5, family = "mont", color = "#193446"),
          legend.text = element_text(family = "open", color = "#676767"),
          plot.title = element_text(face = "bold", family = "mont", color = "#193446"),
          axis.text = element_blank(),
          axis.ticks = element_blank(),
          axis.line = element_blank())
})
...
surf_list[[1]] <- surf_list[[1]] +
  scale_fill_gradientn(colors = parula(100), name = "Secchi Depth (m)  ") +
  labs(title = "")

surf_list[[2]] <- surf_list[[2]] +
  scale_fill_gradientn(colors = parula(100), name = "Mixed-layer Depth (m)") +     # not working inside `labs()`
  labs(title = "")

surf_list[[3]] <- surf_list[[3]] +
  scale_fill_gradientn(colors = parula(100), trans = "log10", name = expression("Total Nitrogen (μg L"^-1*")"),
...
surf_list[[4]] <- surf_list[[4]] +
  scale_fill_gradientn(colors = parula(100), trans = "log10", name = expression("Total Phosphorus (μg L"^-1*")"),
...
top <- quantile(recent_surf$NtP, probs = 0.975, na.rm = TRUE)   # upper bound of color scale
...
surf_list[[5]] <- surf_list[[5]] +
  geom_point(data = recent_surf[recent_surf[["NtP"]] < 16, ],   ### this puts N-limited ponds on top
             ...
             shape = 21) +
  scale_fill_gradientn(colors = c("firebrick4", "firebrick2", parula(11)), name = "N:P Ratio",
                       limits = c(0.01, top),
...
surf_list[[6]] <- surf_list[[6]] +
  scale_fill_gradientn(colors = parula(100), trans = "log10",
                       name = expression("Chorophyll-a (μg L"^-1*")"))

surf_list[[7]] <- surf_list[[7]] +
  scale_fill_gradientn(colors = parula(100), name = "Temperature (°C)   ",
...
surf_list[[9]] <- surf_list[[9]] +
  scale_fill_gradientn(colors = parula(100), name = "DO Saturation (%)",
...
top <- quantile(c(recent_surf$Alk, recent_bott$Alk), probs = 0.95, na.rm = TRUE)   # upper bound of color scale

surf_list[[10]] <- surf_list[[10]] +
  scale_fill_gradientn(colors = parula(100), limits = c(0, top),
                       breaks = c(20, 40, 60),
                       name = expression("Alkalinity (mg L"^-1*")"),
                       na.value = "#F9FB0E")
...
f1b.i <-
  plot_grid(
    plot_grid(
      surf_list[[1]] + theme(legend.position = "none"),
      surf_list[[2]] + theme(legend.position = "none")),
    plot_grid(
      get_legend_35(surf_list[[1]] + theme(legend.key.width = unit(0.45, "in"))),
      get_legend_35(surf_list[[2]] + theme(legend.key.width = unit(0.45, "in")))),
    plot_grid(
      surf_list[[3]] + theme(legend.position = "none"),
      bott_list[[1]] + theme(legend.position = "none")),
    get_legend_35(surf_list[[3]] + theme(legend.key.width = unit(0.95, "in"))),
    plot_grid(
      surf_list[[4]] + theme(legend.position = "none"),
      bott_list[[2]] + theme(legend.position = "none")),
    get_legend_35(surf_list[[4]] + theme(legend.key.width = unit(0.95, "in"))),
    plot_grid(
      surf_list[[5]] + theme(legend.position = "none"),
      surf_list[[6]] + theme(legend.position = "none")),
    plot_grid(
      get_legend_35(surf_list[[5]] + theme(legend.key.width = unit(0.45, "in"),
                                           legend.key.spacing = unit(0.105, "in"))),
      get_legend_35(surf_list[[6]] + theme(legend.key.width = unit(0.45, "in")))),
    nrow = 8,
    rel_heights = c(6, 0.95, 6, 1, 6, 1, 6, 1.5))

ggsave("figures/f1b.i.png", f1b.i, width = 5, height = 14.5, units = "in", dpi = 600, bg = "white")
ggsave("figures/svg/f1b.i.svg", f1b.i, width = 5, height = 14.5, units = "in", bg = "white")

f1b.ii <- plot_grid(
    plot_grid(
      surf_list[[7]] + theme(legend.position = "none"),
      bott_list[[3]] + theme(legend.position = "none")),
    get_legend_35(surf_list[[7]] + theme(legend.key.width = unit(0.95, "in"))),
    plot_grid(
      surf_list[[8]] + theme(legend.position = "none"),
      bott_list[[4]] + theme(legend.position = "none")),
    get_legend_35(surf_list[[8]] + theme(legend.key.width = unit(0.95, "in"))),
    plot_grid(
      surf_list[[9]] + theme(legend.position = "none"),
      bott_list[[5]] + theme(legend.position = "none")),
    get_legend_35(surf_list[[9]] + theme(legend.key.width = unit(0.95, "in"))),
    plot_grid(
      surf_list[[10]] + theme(legend.position = "none"),
      bott_list[[6]] + theme(legend.position = "none")),
    get_legend_35(surf_list[[10]] + theme(legend.key.width = unit(0.95, "in"))),
    nrow = 8,
    rel_heights = c(6, 0.95, 6, 1, 6, 0.95, 6, 1.5))

ggsave("figures/f1b.ii.png", f1b.ii, width = 5, height = 14.5, units = "in", dpi = 600, bg = "white")
ggsave("figures/svg/f1b.ii.svg", f1b.ii, width = 5, height = 14.5, units = "in", bg = "white")
```
...
fish1 <- ggplot() +
  ...
  ylim(41.55, 42.07) +
  theme_classic() +
  theme(text = element_text(family = "open", color = "#193446"),
        panel.grid = element_blank(),
        axis.title = element_blank(),
...
ggsave("figures/f1b.iii.png", f1b.iii, width = 6.5, height = 2.5, units = "in", dpi = 600)
...
Secchi1 <- ggplot(mapping = aes(x = Secchi, y = after_stat(count / sum(count)))) +
  ...
  theme(legend.position = "inside",
        ...
        plot.title = element_text(size = 10, face = "bold", hjust = 0.5, family = "mont", color = "#193446"),
        axis.title = element_text(size = 8, family = "mont", color = "#193446"),
        axis.text = element_text(size = 6, family = "open", color = "#676767"),
        axis.line = element_line(color = "#676767"),
        legend.title = element_blank(),
        legend.margin = margin(0, 1, 1, 1, "pt"),
        legend.text = element_text(size = 6, family = "open", color = "#676767"),
...
Chla1 <- ggplot(mapping = aes(x = log10(Chla), y = after_stat(count / sum(count)))) +
  ...
  scale_x_continuous(breaks = c(-1, 0, 1, 2),
                     labels = 10^c(-1, 0, 1, 2)) +
  labs(x = expression("Chlorophyll-a (μg L"^-1*")"), y = "Proportion", title = "Surface") +
  theme_classic() +
  theme(panel.grid.minor = element_blank(),
        plot.title = element_text(size = 10, face = "bold", hjust = 0.5, family = "mont", color = "#193446"),
        axis.title = element_text(size = 8, family = "mont", color = "#193446"),
...
NtP1 <- ggplot(mapping = aes(x = log10(NtP), y = after_stat(count / sum(count)))) +
  ...
  scale_x_continuous(breaks = c(1, 2, 3),
                     labels = 10^c(1, 2, 3)) +
  labs(x = "N:P Ratio", y = "Proportion", title = "Surface") +
  theme_classic() +
  theme(panel.grid.minor = element_blank(),
        plot.title = element_text(size = 10, face = "bold", hjust = 0.5, family = "mont", color = "#193446"),
        axis.title = element_text(size = 8, family = "mont", color = "#193446"),
...
TN1 <- ggplot(mapping = aes(x = log10(TN), y = after_stat(count / sum(count)))) +
  ...
  scale_x_continuous(limits = c(1.8, 3.8),
                     ...
                     labels = 10^c(2, 3)) +
  ylim(0, 0.13) +
  labs(x = expression("Total Nitrogen (μg L"^-1*")"), y = "Proportion", title = "Surface") +
  theme_classic() +
  theme(panel.grid.minor = element_blank(),
        plot.title = element_text(size = 10, face = "bold", hjust = 0.5, family = "mont", color = "#193446"),
        axis.title = element_text(size = 8, family = "mont", color = "#193446"),
...
TN2 <- ggplot(mapping = aes(x = log10(TN), y = after_stat(count / sum(count)))) +
  ...
  scale_x_continuous(limits = c(1.8, 3.8),
                     ...
                     labels = 10^c(2, 3)) +
  ylim(0, 0.13) +
  labs(x = expression("Total Nitrogen (μg L"^-1*")"), y = "Proportion", title = "Bottom") +
  theme_classic() +
  theme(panel.grid.minor = element_blank(),
        plot.title = element_text(size = 10, face = "bold", hjust = 0.5, family = "mont", color = "#193446"),
        axis.title = element_text(size = 8, family = "mont", color = "#193446"),
...
TP1 <- ggplot(mapping = aes(x = log10(TP), y = after_stat(count / sum(count)))) +
  ...
  scale_x_continuous(limits = c(-0.2, 3.2),
                     ...
                     labels = 10^c(0, 1, 2, 3)) +
  ylim(0, 0.1) +
  labs(x = expression("Total Phosphorus (μg L"^-1*")"), y = "Proportion") +
  theme_classic() +
  theme(panel.grid.minor = element_blank(),
        axis.title = element_text(size = 8, family = "mont", color = "#193446"),
        axis.text = element_text(size = 6, family = "open", color = "#676767"),
...
Temp1 <- ggplot(mapping = aes(x = Temp, y = after_stat(count / sum(count)))) +
  ...
  ylim(0, 0.09) +
  scale_fill_manual(values = colors) +
  labs(x = "Temperature (°C)", y = "Proportion", title = "Surface") +
  theme_classic() +
  theme(legend.position = "inside",
...
Temp2 <- ggplot(mapping = aes(x = Temp, y = after_stat(count / sum(count)))) +
  ...
  xlim(6, 29.4) +
  ylim(0, 0.09) +
  labs(x = "Temperature (°C)", y = "Proportion", title = "Bottom") +
  theme_classic() +
  theme(panel.grid.minor = element_blank(),
        plot.title = element_text(size = 10, face = "bold", hjust = 0.5, family = "mont", color = "#193446"),
        axis.title = element_text(size = 8, family = "mont", color = "#193446"),
...
DO1 <- ggplot(mapping = aes(x = DO, y = after_stat(count / sum(count)))) +
  ...
  xlim(0, 15.75) +
  ylim(0, 0.28) +
  labs(x = expression("Dissolved Oxygen (mg L"^-1*")"), y = "Proportion") +
  theme_classic() +
  theme(panel.grid.minor = element_blank(),
        axis.title = element_text(size = 8, family = "mont", color = "#193446"),
        axis.text = element_text(size = 6, family = "open", color = "#676767"),
...
f2a.i <- plot_grid(Secchi1, MLD1, Chla1, NtP1, TN1, TN2, TP1, TP2, ncol = 2, align = "hv")

ggsave("figures/f2a.i.png", f2a.i, width = 6.5, height = 9, units = "in", dpi = 600)
ggsave("figures/svg/f2a.i.svg", f2a.i, width = 6.5, height = 9, units = "in")

f2a.ii <- plot_grid(Temp1, Temp2, DO1, DO2, DO_sat1, DO_sat2, Alk1, Alk2, ncol = 2, align = "hv")

ggsave("figures/f2a.ii.png", f2a.ii, width = 6.5, height = 9, units = "in", dpi = 600)
ggsave("figures/svg/f2a.ii.svg", f2a.ii, width = 6.5, height = 9, units = "in")
```
...
f2b.i <- plot_grid(Secchi1.1, MLD1.1, Chla1.1, NtP1.1, TN1.1, TN2.1, TP1.1, TP2.1, ncol = 2, align = "hv")

ggsave("figures/f2b.i.png", f2b.i, width = 6.5, height = 9, units = "in", dpi = 600)

f2b.ii <- plot_grid(Temp1.1, Temp2.1, DO1.1, DO2.1, DO_sat1.1, DO_sat2.1, Alk1.1, Alk2.1, ncol = 2, align = "hv")
...
p1 <- ggplot() +
  geom_point(data = eco_annual75, aes(x = Year, y = Secchi), shape = 1, color = "royalblue") +
  geom_point(data = eco_annual25, aes(x = Year, y = Secchi), shape = 1, color = "#b62959") +
  geom_smooth(data = eco_annual75, aes(x = Year, y = Secchi), method = "lm", se = FALSE, col = "royalblue") +
  geom_smooth(data = eco_annual25, aes(x = Year, y = Secchi), method = "lm", se = FALSE, col = "#b62959",
              linetype = "22") +
  geom_hline(yintercept = -4.5, linetype = "dashed") +
  geom_hline(yintercept = -2, linetype = "dashed") +
  annotate("text", x = 2004, y = -0.4, label = "slope: -0.055\nP-value: 0.173", col = "royalblue", size = 3,
           family = "open") +
  annotate("text", x = 2020, y = -0.4, label = "slope: -0.007\nP-value: 0.717", col = "#b62959", size = 3,
           family = "open") +
  ylim(-8, 0) +
  labs(y = "Secchi Depth (m)") +
  theme_linedraw() +
  theme(axis.title = element_text(family = "mont", color = "#193446"),
        axis.text = element_text(family = "open", color = "#676767"),
        axis.line = element_line(color = "#676767"),
        panel.border = element_rect(color = "#676767"))
...
p2 <- ggplot() +
  geom_point(data = eco_annual75, aes(x = Year, y = Chla), shape = 1, color = "royalblue") +
  geom_point(data = eco_annual25, aes(x = Year, y = Chla), shape = 1, color = "#b62959") +
  geom_smooth(data = eco_annual75, aes(x = Year, y = Chla), method = "lm", se = FALSE, col = "royalblue") +
  geom_smooth(data = eco_annual25, aes(x = Year, y = Chla), method = "lm", se = FALSE, col = "#b62959",
              linetype = "22") +
  geom_hline(yintercept = 1, linetype = "dashed") +
  annotate("text", x = 2004, y = 9, label = "slope: 0.08\nP-value: 0.291", col = "royalblue", size = 3,
           family = "open") +
  annotate("text", x = 2020, y = 9, label = "slope: -0.02\nP-value: 0.301", col = "#b62959", size = 3,
           family = "open") +
  scale_y_continuous(limits = c(0, 9.5), breaks = c(0, 2, 4, 6, 8)) +
  labs(y = expression("Chlorophyll-a (μg L"^-1*")")) +
  theme_linedraw() +
  theme(axis.title = element_text(family = "mont", color = "#193446"),
        axis.text = element_text(family = "open", color = "#676767"),
        axis.line = element_line(color = "#676767"),
...
p3 <- ggplot() +
  geom_point(data = eco_annual75, aes(x = Year, y = TN), shape = 1, color = "royalblue") +
  geom_point(data = eco_annual25, aes(x = Year, y = TN), shape = 1, color = "#b62959") +
  geom_smooth(data = eco_annual75, aes(x = Year, y = TN), method = "lm", se = FALSE, col = "royalblue") +
  geom_smooth(data = eco_annual25, aes(x = Year, y = TN), method = "lm", se = FALSE, col = "#b62959",
              linetype = "22") +
  geom_hline(yintercept = 11.42 * 14.0067, linetype = "dashed") +
  annotate("text", x = 2004, y = 600, label = "slope: 5.784\nP-value: 0.166", col = "royalblue", size = 3,
           family = "open") +
  annotate("text", x = 2020, y = 600, label = "slope: 0.1494\nP-value: 0.941", col = "#b62959", size = 3,
           family = "open") +
  ylim(0, 640) +
  labs(y = expression("Total Nitrogen (μg L"^-1*")")) +
  theme_linedraw() +
  theme(axis.title = element_text(family = "mont", color = "#193446"),
        axis.text = element_text(family = "open", color = "#676767"),
        axis.line = element_line(color = "#676767"),
...
p4 <- ggplot() +
  geom_point(data = eco_annual75, aes(x = Year, y = TP), shape = 1, color = "royalblue") +
  geom_point(data = eco_annual25, aes(x = Year, y = TP), shape = 1, color = "#b62959") +
  geom_smooth(data = eco_annual75, aes(x = Year, y = TP), method = "lm", se = FALSE, col = "royalblue") +
  geom_smooth(data = eco_annual25, aes(x = Year, y = TP), method = "lm", se = FALSE, col = "#b62959",
              linetype = "22") +
  geom_hline(yintercept = 0.24 * 30.973762, linetype = "dashed") +
  annotate("text", x = 2004, y = 28, label = "slope: 0.450\nP-value: 0.004", col = "royalblue", size = 3,
           family = "open") +
  annotate("text", x = 2020, y = 28, label = "slope: 0.294\nP-value: <0.001", col = "#b62959", size = 3,
           family = "open") +
  ylim(0, 30) +
  labs(y = expression("Total Phosphorus (μg L"^-1*")")) +
  theme_linedraw() +
  theme(axis.title = element_text(family = "mont", color = "#193446"),
        axis.text = element_text(family = "open", color = "#676767"),
        axis.line = element_line(color = "#676767"),
...
f3 <- plot_grid(p1, p2, p3, p4, ncol = 2, align = "hv")

ggsave("figures/f3.png", f3, width = 6.5, height = 5, units = "in", dpi = 600)
ggsave("figures/svg/f3.svg", f3, width = 6.5, height = 5, units = "in")
```

### Pt. 4: Maps w/ trends in all parameters

Run chunk 24 of PondShiny.Rmd.
...
surf_list2 <- lapply(params, function(param) {
  ggplot() +
    ...
    scale_color_manual(values = c("royalblue", "black", "#b62959"),
                       breaks = c(-1, 0, 1),
                       ...
                       na.translate = FALSE,
                       guide = "none") +
    scale_fill_manual(values = c("royalblue", "black", "#b62959"),
                      breaks = c(-1, 0, 1),
                      limits = factor(c(-1, 0, 1)),
                      labels = c("negative", "zero", "positive"),
                      name = paste(param, "Trend\nDirection"),
...
surf_list2[[3]] <- surf_list2[[3]] +
  labs(title = expression(bold("Surface Total Nitrogen (μg L"^-1*")")))

bott_list2[[1]] <- bott_list2[[1]] +
  labs(title = expression(bold("Bottom Total Nitrogen (μg L"^-1*")")))
  
surf_list2[[4]] <- surf_list2[[4]] +
  labs(title = expression(bold("Surface Total Phosphorus (μg L"^-1*")")))

bott_list2[[2]] <- bott_list2[[2]] +
  labs(title = expression(bold("Bottom Total Phosphorus (μg L"^-1*")")))
...
surf_list2[[7]] <- surf_list2[[7]] +
  labs(title = "Surface Temperature (°C)")

bott_list2[[3]] <- bott_list2[[3]] +
  labs(title = "Bottom Temperature (°C)")

surf_list2[[8]] <- surf_list2[[8]] +
  labs(title = expression(bold("Surface Dissolved Oxygen (mg L"^-1*")")))
  
bott_list2[[4]] <- bott_list2[[4]] +
  labs(title = expression(bold("Bottom Dissolved Oxygen (mg L"^-1*")")))

surf_list2[[9]] <- surf_list2[[9]] +
  labs(title = "Surface DO Saturation (%)") +
  scale_size_continuous(range = c(1, 4),
                        limits = c(0,
                                     sort(abs(c(slopes_surf[["DO_sat"]],
                                                slopes_surf2[["DO_sat"]],
                                                slopes_bott[["DO_sat"]],
                                                slopes_bott2[["DO_sat"]])), decreasing = TRUE)[1]),
                          name = "Trend Magnitude")
...
f4.1 <- plot_grid(
    plot_grid(
      surf_list2[[1]] + theme(legend.position = "none"),
      surf_list2[[2]] + theme(legend.position = "none")),
    plot_grid(
      get_legend_35(surf_list2[[1]] + theme(legend.key.width = unit(0.45, "in"))),
      get_legend_35(surf_list2[[2]] + theme(legend.key.width = unit(0.45, "in")))),
    plot_grid(
      surf_list2[[3]] + theme(legend.position = "none"),
      bott_list2[[1]] + theme(legend.position = "none")),
    get_legend_35(surf_list2[[3]] + theme(legend.key.width = unit(0.45, "in"))),
    plot_grid(
      surf_list2[[4]] + theme(legend.position = "none"),
      bott_list2[[2]] + theme(legend.position = "none")),
    get_legend_35(surf_list2[[4]] + theme(legend.key.width = unit(0.45, "in"))),
    plot_grid(
      surf_list2[[5]] + theme(legend.position = "none"),
      surf_list2[[6]] + theme(legend.position = "none")),
    plot_grid(
      get_legend_35(surf_list2[[5]] + theme(legend.key.width = unit(0.45, "in"))),
      get_legend_35(surf_list2[[6]] + theme(legend.key.width = unit(0.45, "in")))),
    nrow = 8,
    rel_heights = c(6, 1.75, 6, 1.75, 6, 1.75, 6, 1.75))

ggsave("figures/f4.1.png", f4.1, width = 9, height = 14.5, units = "in", dpi = 600, bg = "white")
ggsave("figures/svg/f4.1.svg", f4.1, width = 9, height = 14.5, units = "in", bg = "white")

f4.2 <- plot_grid(
    plot_grid(
      surf_list2[[7]] + theme(legend.position = "none"),
      bott_list2[[3]] + theme(legend.position = "none")),
    get_legend_35(surf_list2[[7]] + theme(legend.key.width = unit(0.45, "in"))),
    plot_grid(
      surf_list2[[8]] + theme(legend.position = "none"),
      bott_list2[[4]] + theme(legend.position = "none")),
    get_legend_35(surf_list2[[8]] + theme(legend.key.width = unit(0.45, "in"))),
    plot_grid(
      surf_list2[[9]] + theme(legend.position = "none"),
      bott_list2[[5]] + theme(legend.position = "none")),
    get_legend_35(surf_list2[[9]] + theme(legend.key.width = unit(0.45, "in"))),
    plot_grid(
      surf_list2[[10]] + theme(legend.position = "none"),
      bott_list2[[6]] + theme(legend.position = "none")),
    get_legend_35(surf_list2[[10]] + theme(legend.key.width = unit(0.45, "in"))),
    nrow = 8,
    rel_heights = c(6, 1.75, 6, 1.75, 6, 1.75, 6, 1.75))

ggsave("figures/f4.2.png", f4.2, width = 9, height = 14.5, units = "in", dpi = 600, bg = "white")
ggsave("figures/svg/f4.2.svg", f4.2, width = 9, height = 14.5, units = "in", bg = "white")

f4.legend <- plot_grid(get_legend_35(surf_list2[[1]] +
                          geom_point(show.legend = TRUE) +
                          scale_fill_discrete(breaks = c(-2, -1, 0, 1, 2),
                                            limits = factor(c(-2, -1, 0, 1, 2)),
                                            labels = c("negative, significant", "negative, non-significant",
                                                       "zero", "positive, non-significant",
                                                       "positive, significant"),
                                            name = "Trend Direction\n& Significance",
                                            drop=FALSE) +
                          guides(fill = guide_legend(title.hjust = 0,
                                                     override.aes = list(shape = 21,
                                                                         fill = c("royalblue", "white",
                                                                                  "white", "white",
                                                                                  "#b62959"),
                                                                         col = c("black", "royalblue", 
                                                                                 "black", "#b62959",
                                                                                 "black")))) +
                          theme(legend.position = "right"))[5])

ggsave("figures/f4.legend.png", f4.legend, dpi = 600)
ggsave("figures/svg/f4.legend.svg", f4.legend)
```
