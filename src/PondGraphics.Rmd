...
ponds <- ponds %>%
  group_by(Station.name, Year) %>%
  mutate(NtPClass = ifelse(mean(TN, na.rm = TRUE) / mean(TP, na.rm = TRUE) < 16,
                           "N-limited", "P-limited"))
```
...
f1a.i.i <- ggplot(pie, aes(x = "", y = n, fill = factor(bin, levels = c("[1,5]", "(5,10]", "(10,15]", "> 15")))) +
...
ggsave("figures/f1a.i.i.png", f1a.i.i, width = 4, height = 3, units = "in", dpi = 600)
```
...
surf_list[[1]] <- surf_list[[1]] +
  guides(fill = guide_colorbar(title = "Secchi\nDepth (m)"))

surf_list[[2]] <- surf_list[[2]] +
  scale_fill_gradientn(colors = parula(100), trans = "log10", name = expression("Chla (μg L"^-1*")"))
  
surf_list[[3]] <- surf_list[[3]] +
  scale_fill_gradientn(colors = parula(100), trans = "log10", name = expression("TN (μg L"^-1*")"),
                       limits = c(sort(c(recent_surf$TN,
                                         recent_bott$TN))[1],
                                  sort(c(recent_surf$TN,
                                         recent_bott$TN), decreasing = TRUE)[1]))

bott_list[[1]] <- bott_list[[1]] +
  scale_fill_gradientn(colors = parula(100), trans = "log10", expression("TN (μg L"^-1*")"),
                       limits = c(sort(c(recent_surf$TN,
                                         recent_bott$TN))[1],
                                  sort(c(recent_surf$TN,
                                         recent_bott$TN), decreasing = TRUE)[1]))
...
surf_list[[4]] <- surf_list[[4]] +
  scale_fill_gradientn(colors = parula(100), trans = "log10", expression("TP (μg L"^-1*")"),
                       limits = c(sort(c(recent_surf$TP,
                                         recent_bott$TP))[2],
                                  sort(c(recent_surf$TP,
                                         recent_bott$TP), decreasing = TRUE)[1]))
...
surf_list[[8]] <- ggplot() +
    geom_sf(data = mass) +
    geom_point(data = recent_surf[recent_surf$TN_flag == 2 & recent_surf$TP_flag == 2, ],
               aes(x = Long, y = Lat, fill = log(TN / TP)),
               shape = 21) +
    scale_fill_gradientn(colors = parula(100),
                         limits = c(sort(c(recent_surf[[param]],
                                           recent_bott[[param]]))[1],
...
surf_list[[7]] <- surf_list[[7]] +
  scale_fill_gradientn(colors = parula(100), limits = c(0, 41), name = expression("Alkalinity\n(mg L"^-1*")"))
