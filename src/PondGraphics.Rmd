...
### Temporary: Fish space map
```{r}
ggplot() +
  geom_point(data = ponds, aes(x = FishSpace, y = FishSpace * Size * 4046.86)) +
...
fish1 <- ggplot() +
  geom_sf(data = mass) +
  geom_point(data = ponds[ponds$FishSpace == 0, ],
             aes(x = Long, y = Lat), fill = "firebrick2",
             shape = 21) +
  geom_point(data = ponds[(!is.na(ponds$FishSpace) & ponds$FishSpace != 0), ],
             aes(x = Long, y = Lat, fill = log10(FishSpace)),
             shape = 21) +
  scale_fill_gradientn(colors = parula(100),
                       breaks = c(log10(1), log10(3), log10(10)),
                       labels = c(1, 3, 10),
                       name = "Vertical\nFish Space (m)") +
  xlim(-70.65, -69.95) +
  ylim(41.55, 42.07) +
...
fish2 <- ggplot() +
  ...
  geom_point(data = ponds[ponds$FishSpace == 0, ],
             ...
             shape = 21) +
  geom_point(data = ponds[(!is.na(ponds$FishSpace) & ponds$FishSpace != 0), ],
             aes(x = Long, y = Lat, fill = log10(FishSpace * Size * 4046.86)),    # conversion to m^2
             shape = 21) +
  scale_fill_gradientn(colors = parula(100),
                       breaks = c(log10(3*10^4), 5, log10(3*10^5), 6, log10(3*10^6)),
                       labels = c(3*10^4, 1*10^5, 3*10^5, 1*10^6, 3*10^6),
...
f1a.v <- plot_grid(fish1, fish2,
                   ncol = 2, nrow = 1, align = "hv")

ggsave("figures/f1a.v.png", f1a.v, width = 6.5, height = 2.5, units = "in", dpi = 600, bg = "white")
```
...
DO_sat1 <- ggplot(mapping = aes(x = DO_sat, y = after_stat(count / sum(count)))) +
  ...
  #xlim(-0.25, 14.75) +
  #ylim(0, 0.28) +
  labs(x = expression("DO Saturation (%)"), y = "Proportion") +
  theme_classic() +
  theme(panel.grid.minor = element_blank(),
        axis.title = element_text(size = 8),
        axis.text = element_text(size = 6))
...
Alk1 <- ggplot(mapping = aes(x = Alk, y = after_stat(count / sum(count)))) +
  ...
  # xlim(0.5, 30.5) +
  # ylim(0, 0.115) +
  labs(x = "Alkalinity (mg L"^-1*")", y = "Proportion") +
  theme_classic() +
  theme(panel.grid.minor = element_blank(),
        axis.title = element_text(size = 8),
        axis.text = element_text(size = 6))
...
f1 <- ggarrange(Secchi1, MLD1, Chla1, TN1, TN2, TP1, TP2, Temp1, Temp2, DO1, DO2, ncol = 2)

# DO_sat1, DO_sat2, Alk1, Alk2
