...
for(pond in pondName) {
  onePond <- ponds %>%
    ...
    group_by(Station.name, Station.number, Site, Lens, PercentBuffer, BufferClass, Year, Depth) %>%
  ...
  for(year in unique(onePond$Year)) {
    for(j in 1:nrow(onePond[onePond$Year == year, ])) {
      ...
      if(!is.na(onePond$Temp[onePond$Year == year & onePond$Depth == depth]) &
        ...
        for(k in 17:(ncol(onePond) - 4)) {   ### change here when cols change
  ...
  for(i in 12:(ncol(oneSurface) - 4)) {   ### change here when cols change
...
allSurface <- allSurface %>%
  mutate(DepthClass = case_when(Final.Depth < 3 ~ "Shallow",
                                Final.Depth > 3 ~ "Deep")) %>%
  relocate(DepthClass, .after = Final.Depth)
