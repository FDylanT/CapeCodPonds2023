...
First run chunks 2-3 and 6-7 of `PondShiny.Rmd` script.
...
for(pond in pondName) {
  onePond <- ponds %>%
    ...
    group_by(Station.name, Station.number, Site, DepthClass, SizeClass, Lens, PercentBuffer, BufferClass,
             Year, Depth) %>%
  ...
  for(year in unique(onePond$Year)) {
    for(j in 1:nrow(onePond[onePond$Year == year, ])) {
      ...
      if(!is.na(onePond$Temp[onePond$Year == year & onePond$Depth == depth]) &
        ...
        for(k in 19:(ncol(onePond) - 4)) {   ### change here when cols change
  ...
  for(i in 14:(ncol(oneSurface) - 4)) {   ### change here when cols change
...
for(pond in pondName) {
  ...
  for(year in unique(onePond$Year)) {
    for(j in 1:nrow(onePond[onePond$Year == year, ])) {
      ...
      if(!is.na(onePond$Temp[onePond$Year == year & onePond$Depth == depth]) &
         !is.na(onePond$DO[onePond$Year == year & onePond$Depth == depth])) {
        for(l in 19:(ncol(onePond) - 4)) {   ### change here when cols change
  ...
  if(nrow(oneBottom) == 0) {
    ...
    for(i in 16:(ncol(oneBottom) - 4)) {   ### change here when cols change
