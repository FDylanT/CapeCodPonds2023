...
# "TN_uM" is the last, discrete-sample columns begin after "DO_sat", and
...
for(pond in pondName) {
  onePond <- focal_ponds %>%
    ...
    group_by(Station.name, Lens, DepthClass, StratClass, SurfaceAreaClass, DevelopmentClass,
             Year, Depth) %>%
  ...
  param_end <- which(colnames(onePond) == "TN_uM")
  ...
  oneSurface$NtPClass <- ifelse(mean(oneSurface$TN) / mean(oneSurface$TP) < 16,
                                "N-limited",
                                "P-limited")
...
for(pond in pondName) {
  ...
  oneBottom$NtPClass <- ifelse(mean(oneSurface$TN) / mean(oneSurface$TP) < 16,
                                "N-limited",
                                "P-limited")
