...
## Attempt to identify temp & DO profile outliers
```{r}
outlier_frame_profiles <- data.frame(matrix(ncol = 2, nrow = 500))
colnames(outlier_frame_profiles) <- colnames(ponds)[12:13]
...
for(pond in pondName) {
  ...
  for(param in colnames(onePond)[12:13]) {
    ...
    if(length(outliers) > 0) {
      outlier_frame_profiles[[param]][sum(!is.na(outlier_frame[[param]])) +
                                        1:length(outliers)] <- outliers
    }
  }
}
```
