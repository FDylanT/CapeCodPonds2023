...
for(i in 1:nrow(ponds)) {
  if(is.na(ponds$DO_sat[i])) {
    ...
    ponds$DO_sat[i] <- ponds$DO[i] / 15.999 / rho * 10^6 /
...
DT::renderDataTable({
  ponds[ponds$Station.name == input$pond, c(1, 7:9, 12:13)] %>%
...
renderPlot({
  ...
  ponds[ponds$Station.name == input$pond &
        ponds[[flag_name]] == 2, ] %>%
  ggplot(aes(x = get(anom_name), y = Depth, fill = Year)) +
    geom_point(size = 4, shape = 21) +
