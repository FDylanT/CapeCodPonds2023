...
for(pond in allPonds) {
  ...
  for(y in unique(onePond$Year)) {
    oneYear <- onePond[onePond$Year == y &
                       ...
                       !is.na(onePond$Depth), ]
    if(sum(!is.na(oneYear$Temp)) < 2) {           # removed: | n_distinct(oneYear$Depth[oneYear$Depth >= 1]) < 2
      next
    }
    for(i in 1:(n_distinct(oneYear$Depth) - 1)) {
      if(is.na(oneYear$Temp[i])) {                # removed: | oneYear$Depth[i] < 1
        next
      }
      j <- i + 1
...
# define nutrient limitation
ponds <- ponds %>%
  group_by(Station.name, Year) %>%
  mutate(NtPClass = ifelse(mean(TN_uM[Depth <= 1], na.rm = TRUE) / mean(TP_uM[Depth <= 1], na.rm = TRUE) < 16,
                           "N-limited", "P-limited")) %>%
  ungroup
```
...
selectInput("pond", label = "Choose sampling station:", choices = pondName)
```
...
# min(log10(surface_recent$TN_uM / surface_recent$TP_uM), na.rm = TRUE)
# max(log10(surface_recent$TN_uM / surface_recent$TP_uM), na.rm = TRUE)

renderPlot({
  surface_recent <- allSurface %>%
    group_by(Station.name) %>%
    filter(Year == max(Year)) %>%
    mutate(TN_uM = if_else(TN_uM == 0, NA, TN_uM),
           TP_uM = if_else(TP_uM == 0, NA, TP_uM))
  ggplot() +
    geom_sf(data = mass) +
...
limits <- allBottom %>%
  mutate(layer = "b",
         Chla = NA) %>%
  rbind(cbind(allSurface, layer = "s")) %>%
  filter(Year >= 2016) %>%
