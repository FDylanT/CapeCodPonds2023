...
ponds <- ponds %>%
  ...
  relocate(TN, .after = Alk) %>%
  relocate(TP, .after = TN) %>%
...
for(i in 1:nrow(ponds)) {
  ...
  if(length(chars$Final.Depth[chars$CCC_GIS_ID == ID]) > 0) {
    ...
    ponds$DevelopmentClass[i] <- chars$buffer.development.class[chars$CCC_GIS_ID == ID]
  }
}
...
for(pond in allPonds) {
  ...
  # temporary; covers ponds without "final depth" in characteristics datasheet
  if(is.na(onePond$Final.Depth[1]) & !is.na(onePond$Total.Depth[1])) {
    ponds$Final.Depth[ponds$Station.name == pond] <- max(onePond$Total.Depth, na.rm = TRUE)
  }
  
  # second half temporary; covers ponds with no "final depth" or "total depth" data
  if(is.na(ponds$Final.Depth[ponds$Station.name == pond][1])) {
    ponds$Final.Depth[ponds$Station.name == pond] <- max(onePond$Depth, na.rm = TRUE)
  }
}

ponds <- ponds %>%
  relocate(MLD, .before = Depth) %>%
  relocate(Final.Depth, .before = MLD)
...
```{r include = FALSE}
### determine stratified vs unstratified ponds
ponds$Strat <- ifelse(ponds$MLD <= 0.75 * ponds$Final.Depth, "Stratified", "Mixed")

ponds <- ponds %>%
  relocate(Strat, .after = MLD)
```
...
```{r echo = FALSE}
props <- c("Secchi", "Temp", "DO", "Alk", "TN", "TP", "Chla")
...
renderPlot({
  ...
  class3 <- case_when(input$class3 == "None" ~ "None",
                      input$class3 == "Stratification" ~ "Strat",
  ...
  class4 <- case_when(input$class4 == "Stratification" ~ "Strat",
...
renderUI({
  choices <- if(input$layer == "Surface") {
                c("Secchi", "Temp", "DO", "DO_sat", "Alk", "TN", "TP", "Chla")
              } else {
                c("Temp", "DO", "DO_sat", "Alk", "TN", "TP")
              }
...
start <- which(colnames(allSurface) == "Secchi")      ###### add MLD to these lists of params
...
```{r echo = FALSE}
selectInput("param6", label = "Choose parameter:", choices = c("Secchi", "Temp", "DO", "DO_sat", "Alk", "TN", "TP", "Chla"))
...
selectInput("prop5", label = "Choose x parameter:", choices = c("Secchi", "Temp", "DO", "Alk", "TN", "TP", "Chla"))
