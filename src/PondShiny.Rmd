...
classes <- c("Depth", "% Buffer Developed", "Surface Area", "Lens")
...
renderPlot({
  ...
  class1 <- case_when(input$class1 == "Depth" ~ "DepthClass",
...
renderPlot({
  ...
  p <- allBottom[allBottom[[flag_name1]] == 2 &
             ...
             !is.na(allBottom[[class1]]) &
             !is.na(allBottom[[class2]]) &
             allBottom$Year >= input$yearRange[1] &
             allBottom$Year <= input$yearRange[2], ] %>%
  ggplot(aes(x = get(input$prop3), y = get(input$prop4), col = get(class2))) +
    geom_point(size = 2.5, shape = 1) +
    facet_wrap(~ get(class1)) +
    labs(x = input$prop3, y = input$prop4, col = input$class4) +
    theme_bw() +
    theme(panel.grid = element_blank())
    if(input$prop3 == "TP" | input$prop3 == "TN" | input$prop3 == "Chla") {
      p <- p + scale_x_log10()
    }
    if(input$prop4 == "TP" | input$prop4 == "TN" | input$prop4 == "Chla") {
      p <- p + scale_y_log10()
    }
