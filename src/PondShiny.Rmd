...
ponds <- ponds[-which(ponds$Alk == "n.m. 19.90"), ]
...
ponds$PercentBuffer <- as.numeric(str_extract(ponds$PercentBuffer, ".*(?=%)"))

# ponds_grouped <- ponds %>%
#   group_by(Station.name) %>%
#   summarize(PercentBuffer = mean(PercentBuffer))
# 
# quantile(ponds_grouped$PercentBuffer, probs = c(0.25, 0.5, 0.75), na.rm = TRUE)

ponds <- ponds %>%
  mutate(BufferClass = case_when(ponds$PercentBuffer <= 5.00 ~ "Very Low",
                                 ponds$PercentBuffer <= 13.00 ~ "Low",
                                 ponds$PercentBuffer <= 23.25 ~ "Medium",
                                 ponds$PercentBuffer > 23.25 ~ "High"))
...
#                                           "DO_sat (%)", "Alkalinity (mg/L)", "TP (μM)",
#                                           "TN (μM)", "Chlorophyll-a (μg/L)")
...
#### test code
```{r}
pond <- "HA-416-01 / Josephs Pond"

flag_name <- "Temp_flag"
param <- "Temp"

allSurface[allSurface$Station.name == pond &
             allSurface[[flag_name]] == 2, ] %>%
  ggplot(aes(x = Year, y = get(param))) +
    geom_point(size = 2.5) +
    labs(x = "Year", y = paste("Surface", param)) +
    theme_bw() +
    if(FALSE) {
      mk <- mk.test(allSurface[[param]][allSurface$Station.name == pond &
                                              allSurface[[flag_name]] == 2])
      slope <- sens.slope(allSurface[[param]][allSurface$Station.name == pond &
                                          allSurface[[flag_name]] == 2])$estimates
      intercept <- median(allSurface[[param]][allSurface$Station.name == pond &
                                          allSurface[[flag_name]] == 2]) - 
                   median(allSurface$Year[allSurface$Station.name == pond &
                                          allSurface[[flag_name]] == 2]) * slope
      list(
        geom_abline(slope = slope, intercept = intercept, col = "black", linewidth = 1),
        annotate("text",
                 x = min(allSurface$Year[allSurface$Station.name == pond &
                                        allSurface[[flag_name]] == 2]) + 1,
                 y = max(allSurface[[param]][allSurface$Station.name == pond &
                                              allSurface[[flag_name]] == 2]) * 0.95,
                 label = paste("slope =", round(slope, 3)), col = "black"),
        annotate("text",
                 x = min(allSurface$Year[allSurface$Station.name == pond &
...
```{r echo = FALSE}
start <- which(colnames(allSurface) == "Secchi") + 1
end <- which(colnames(allSurface) == "Total.Depth") - 1
...
renderUI({
  ...
  selectInput("prop2", label = "Choose y parameter:",
              choices = props[!(props %in% input$prop1)])   ### why isn't Chla showing up
})
...
start <- which(colnames(allBottom) == "Secchi") + 1
...
```{r echo = FALSE}
selectInput("param7", label = "Choose parameter:", choices = c("Temp", "DO", "DO_sat", "Alk", "TP", "TN"))
