...
for(pond in allPonds) {
  ID <- str_extract(pond, "\\(.*")
  if(sum(str_detect(allSurface$Station.name, ID)) == 0) {
    next
  }
  ...
  for(d in unique(onePond$Date)) {
    if(str_extract(d, "^\\d{1,2}") != "8" & str_extract(d, "^\\d{1,2}") != "9") {
      next
    }
    if(sum(allSurface$Year[allSurface$Station.name == pond &
                           !is.na(allSurface$MLD)] == str_extract(d, "\\d{4}")) == 0) {
      next
    }
    if(allSurface$StratClass[allSurface$Station.name == pond &
                             allSurface$Year == str_extract(d, "\\d{4}")] != "Stratified") {
      next
    }
    
    oneDate <- onePond[onePond$Date == d &
                       (onePond$Month == 8 |      # delete this + next line to eval. MLD in all months
                        onePond$Month == 9) &     # note that doing this^^ will mess up plots
                       !is.na(onePond$Depth), ] %>%
      arrange(Depth)
    if(sum(!is.na(oneDate$Temp)) < 2 | n_distinct(oneDate$Depth[oneDate$Depth >= 1]) < 2) {
      next
    }
    index <- tail(which((oneDate$Temp - lag(oneDate$Temp)) / (oneDate$Depth - lag(oneDate$Depth)) ==
      min((oneDate$Temp - lag(oneDate$Temp)) / (oneDate$Depth - lag(oneDate$Depth)), na.rm = TRUE)), 1)
    # if(length(index) > 1) {
    #   print(paste(pond, "--", d))
    # }
    ...
    if(length(which(oneDate$DO >= 4)) > 0) {
      ponds$DO.4[ponds$Station.name == pond & ponds$Date == d] <-
        oneDate$Depth[tail(which(oneDate$DO >= 4), 1)]
    }
    ponds$FishSpace <- ifelse(ponds$DO.4 - ponds$hypo > 0, ponds$DO.4 - ponds$hypo, 0)
  }
}

ponds <- ponds %>%
  relocate(hypo, .after = MLD) %>%
  relocate(DO.4, .after = hypo) %>%
  relocate(FishSpace, .after = DO.4)
```
...
#     filter(Year >= 2016) %>%
#     group_by(Station.name) %>%
#     summarise(across(where(is.numeric) & !Year, ~ mean(.x, na.rm = TRUE))) %>%   # don't average the actual years
#                                                                                  # (e.g. 2016 + 2021)
...
limits <- allBottom %>%
  mutate(layer = "b",
         Chla = NA) %>%
  rbind(cbind(allSurface, layer = "s")) %>%  # bind allS & allB
  filter(Year >= 2016) %>%
...
DT::renderDataTable({
  if(input$layer2 == "Surface") {
    trend_data <- allSurface %>%
      filter(!is.na(get(paste0(input$trend, "_trend")))) %>%  # remove observations not used for earlier trend calcs
