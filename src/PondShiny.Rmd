...
renderPlot({
  allSurface %>%
    filter(Station.name == input$pond) %>%
    filter(Chla_flag == 2) %>%
    #filter(Secchi_flag == 2) %>%  # doesn't exist yet
...
```{r}
param <- colnames(allSurface)[c(8:14, 16:18)][7]

flag_name <- paste0(param, "_flag")
allSurface[allSurface$Station.name == "Hinckley Pond / BA-411-01" &
           allSurface[[flag_name]] == 2, ] %>%
ggplot(aes(x = Year, y = get(param))) +
  ...
  theme_bw() +
  if(TRUE) {
    slope <- sens.slope(allSurface[[param]][allSurface$Station.name == "Hinckley Pond / BA-411-01" &
                                        allSurface[[flag_name]] == 2])$estimates
    intercept <- median(allSurface[[param]][allSurface$Station.name == "Hinckley Pond / BA-411-01" &
                                        allSurface[[flag_name]] == 2]) - 
                 median(allSurface$Year[allSurface$Station.name == "Hinckley Pond / BA-411-01" &
                                        allSurface[[flag_name]] == 2]) * slope
    geom_abline(slope = slope, intercept = intercept, col = "firebrick2", linewidth = 1)
  }
```
