...
for(i in 1:length(pondID)) {
  programs <- unique(ponds$Measuring.program.name[ponds$Station.number == pondID[i]])
  program[i] <- paste(programs, collapse = ", ")
}

# create data frame
pond_summary <- as.data.frame(cbind(pondID, pond_name, years_data, years_profiles,
                                    max_per_year, avg_per_year, all_years, params, 
                                    param_years, last_year, param_last_values,
                                    program)) %>%
  mutate_if(is_all_numeric, as.numeric)
```
...
## Write csv
```{r}
write.csv(pond_summary, "data/PondsSummary09142023.csv", row.names = FALSE)
```
...
surface_recent <- surface %>%
  group_by(Station.number) %>%
  filter(Year == max(Year)) %>%
  mutate(ratioNP = TN / TP)
...
ggmap(CC_map) +
  ...
  scale_color_gradientn(colors = parula(100), name = "TN:TP", trans = "log10",
                        limits = c(1, 3.3), breaks = c(1, 2, 3), labels = c(10, 100, 1000)) +
  coord_sf(xlim = c(-70.65, -69.95), ylim = c(41.55, 42.07)) +
  theme_bw() +
  theme(axis.title = element_blank())
...
ggplot(MWP_surface[MWP_surface$Secchi_flag == 2, ], aes(x = Year, y = Secchi)) +
  ...
  annotate("text", x = 2003, y = 2.1,
           label = paste0("slope = ", round(slope, 3)), col = "firebrick2") +
  annotate("text", x = 2003, y = 1.9,
...
model <- lm(Chlorophyll ~ Year, MWP_surface[MWP_surface$Chlorophyll_flag == 2, ])
summary(model)
mk <- mk.test(MWP_surface$Chlorophyll[MWP_surface$Chlorophyll_flag == 2]) # p = 0.9641
slope <- sens.slope(MWP_surface$Chlorophyll[MWP_surface$Chlorophyll_flag == 2])$estimates
intercept <- median(MWP_surface$Chlorophyll[MWP_surface$Chlorophyll_flag == 2]) - 
  median(MWP_surface$Year[MWP_surface$Chlorophyll_flag == 2]) * slope

ggplot(MWP_surface[MWP_surface$Chlorophyll_flag == 2, ], aes(x = Year, y = Chlorophyll)) +
  geom_point(size = 2.5) +
  geom_smooth(method = "lm", se = FALSE, col = "royalblue") +
